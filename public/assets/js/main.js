...
// Prevent redeclaration errors on navigation
if (typeof window.mainJSInitialized === 'undefined') {
  window.mainJSInitialized = true;

  window.isRtl = window.Helpers.isRtl();
  window.isDarkStyle = window.Helpers.isDarkStyle();
  window.mainMenuInstance = null;
  window.mainAnimate = null;
  window.mainIsHorizontalLayout = false;
}

// Use window object directly to avoid redeclaration errors
// Don't use var/let/const - just reference window properties directly

// Function to initialize JavaScript after navigation
function initializeMainJS() {
  // Update isHorizontalLayout based on current DOM
  if (document.getElementById('layout-menu')) {
    window.mainIsHorizontalLayout = document.getElementById('layout-menu').classList.contains('menu-horizontal');
  }
...
// Accordion active class function (needs to be accessible globally)
...
function initMainComponents() {
  ...
  if (layoutMenuEl.length > 0) {
    layoutMenuEl.forEach(function (element) {
      if (window.Helpers.mainMenu) {
        try {
          if (typeof window.Helpers.mainMenu.destroy === 'function') {
      ...
      try {
        window.mainMenuInstance = new Menu(element, {
          orientation: window.mainIsHorizontalLayout ? 'horizontal' : 'vertical',
          closeChildren: window.mainIsHorizontalLayout ? true : false,
        ...
        window.Helpers.scrollToActive((window.mainAnimate = false));
        window.Helpers.mainMenu = window.mainMenuInstance;
      } catch (e) {
        console.warn('Error initializing menu:', e);
      }
    });
  }

  // Reinitialize menu togglers
  ...
  menuToggler.forEach(item => {
    ...
    if (item.parentNode) {
      ...
      newItem.addEventListener('click', event => {
        ...
        if (typeof config !== 'undefined' && config.enableMenuLocalStorage && !window.Helpers.isSmallScreen()) {
          try {
            ...
            if (layoutCollapsedCustomizerOptions) {
              ...
              const input = layoutCollapsedCustomizerOptions.querySelector(`input[value="${layoutCollapsedVal}"]`);
  ...
  if (notificationMarkAsReadAll) {
    const newNotificationAll = notificationMarkAsReadAll.cloneNode(true);
    if (notificationMarkAsReadAll.parentNode) {
      notificationMarkAsReadAll.parentNode.replaceChild(newNotificationAll, notificationMarkAsReadAll);
      newNotificationAll.addEventListener('click', event => {
        document.querySelectorAll('.dropdown-notifications-read').forEach(item => {
          const itemParent = item.closest('.dropdown-notifications-item');
          if (itemParent) itemParent.classList.add('marked-as-read');
        });
      });
    }
  }
  if (notificationMarkAsReadList.length > 0) {
    notificationMarkAsReadList.forEach(item => {
      const newItem = item.cloneNode(true);
      if (item.parentNode) {
        item.parentNode.replaceChild(newItem, item);
        newItem.addEventListener('click', event => {
          const itemParent = newItem.closest('.dropdown-notifications-item');
          if (itemParent) itemParent.classList.toggle('marked-as-read');
        });
      }
    });
  }

  // Reinitialize theme toggles
  ...
  // Reinitialize search if it exists
  ...
  console.log('Main components initialized successfully');
}
...
document.addEventListener('livewire:init', function () {
  ...
  // Also listen for navigated event
...
// Fallback: also listen directly for navigated (in case livewire:init already fired)
document.addEventListener('livewire:navigated', function () {
  console.log('Livewire navigated (direct), reinitializing...');
  setTimeout(() => {
    initMainComponents();
  }, 100);
});

// Also reinitialize on page show (for browser back/forward)
window.addEventListener('pageshow', function(event) {
  if (event.persisted) {
    console.log('Page shown from cache, reinitializing...');
    setTimeout(() => {
      initMainComponents();
    }, 100);
  }
...
(function () {
          ...
          if (window.innerWidth < window.Helpers.LAYOUT_BREAKPOINT) {
            if (document.getElementById('layout-menu')) {
              if (document.getElementById('layout-menu').classList.contains('menu-horizontal')) {
                if (window.mainMenuInstance && typeof window.mainMenuInstance.switchMenu === 'function') {
                  window.mainMenuInstance.switchMenu('vertical');
                }
              }
            }
          } else {
            if (document.getElementById('layout-menu')) {
              if (document.getElementById('layout-menu').classList.contains('menu-vertical')) {
                if (window.mainMenuInstance && typeof window.mainMenuInstance.switchMenu === 'function') {
                  window.mainMenuInstance.switchMenu('horizontal');
                }
              }
            }
          }
  ...
  if (window.mainIsHorizontalLayout || window.Helpers.isSmallScreen()) {
    return;
  }
